...
const Table = React.forwardRef<TableRef, TableProps>((props, ref) => {
  ...
  const handleChangPage = useCallback((page: number, limit: number) => onPageChange?.(page, limit), [onPageChange]);
  ...
  const handleSelectionChange = useCallback(
    (item: any | "all") => {
      let onSelectionChange = refOnSelectionChange.current;
      if (!onSelectionChange || !item) return;
      let items = refItems.current;
      let uniqueKey = refUniqueKey.current;
      let selectedItems = refSelectedItems.current;
      if (item === "all") {
  ...
  const renderHeadCell = useCallback(
    (col: ReactElement, depth: number, compareProps: { isSubCell: boolean; isShow: boolean }): TableCellProps => {
      const cell = {
      ...
      if (cell.children) {
        ...
        if (isArray(cell.children)) {
          ...
          if ((cell.children as any)?.type?.displayName && (cell.children as any).type.displayName === "TableColumn") {
            cell.subCells?.push(
              renderHeadCell(cell.children as ReactElement, depth + 1, { isSubCell: true, isShow: cell.isShow })
            );
          }
        }
        if (settingColumns && cell.subCells && cell.subCells.length > 0) {
          cell.subCells.sort((a, b) => {
            return Object.keys(settingColumns).indexOf(a.field) - Object.keys(settingColumns).indexOf(b.field);
          });
        }
      }
  ...
  return (
    <Fragment>
      <TableContext.Provider value={valueContext}>
        <TableContainer ref={headerRef} sticky={sticky} stickyPosition={stickyPosition} boxShadow={!!fixTopTableRowV2}>
          <StyledTable>
            {_ColGroup}
        ...
        <TableContainer tableTheme={props.tableTheme}>
          <Scrollbars
            ref={scrollbarRef}
            autoHeight
            autoHeightMax={maxHeightScroll || "unset"}
            renderTrackHorizontal={({ style, ...props }) =>
              fixScrollXBar ? (
                <TrackHorizontal
                  {...props}
                  style={{ ...style, position: "fixed", height: 12, left: 0, right: 0, ...scrollXStyle, zIndex: 10 }}
                />
              ) : (
                <TrackHorizontal {...props} style={{ ...style, left: 0, right: 0, ...scrollXStyle }} />
              )
            }
...
const ColGroup = (props: ColGroupProps) => {
  ...
  return (
    <colgroup>
      ...
      {cols.map((col, index) => (
        <col key={index} style={{ width: col.width, maxWidth: col.maxWidth, minWidth: col.minWidth }} />
      ))}
    </colgroup>
  );
};
